...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#123456") {
        canvas = canvas_pass
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    let buildings = new Image()
    buildings.src = "buildings.png"

    class Building{
        constructor(){
            this.width = Math.floor(Math.random()*5)+2
            this.height = Math.floor(Math.random()*19)+3
            this.grid = []
            this.x = Math.random()*1280

            this.xpointer = this.x-(this.width*.5*16)
            this.ypointer = 700-(this.height*16)
            for(let t = 0;t<this.height;t++){
                this.xpointer = this.x-(this.width*.5*16)
                    for(let k = 0;k<this.width;k++){
                        let square = new Rectangle(this.xpointer, this.ypointer, 16, 16, "transparent")
                        square.type = Math.floor(Math.random()*4)
        ...
        draw(){
            ...
            //         for(let k = 0;k<this.width;k++){
                for(let t=  0;t<this.grid.length;t++){
                    if(this.grid[t].xmom != 0 || this.grid[t].ymom != 0 ){
                        this.grid[t].ymom += .2
                        if(t >= this.width){
                            this.grid[t-this.width].ymom += .01
                        }
                        this.grid[t].type = 0
                    }else{
                        if(Math.random()<.01){
                            this.grid[t].type = Math.floor(Math.random()*4)
                        }
                    }
                    this.grid[t].move()
                    if(t == 0){
                        if(t%this.width == 0){
                            canvas_context.drawImage(buildings, this.grid[t].type*16, 16, 16,16, this.grid[t].x, this.grid[t].y, 16,16)
                        }else if(t%this.width == this.width-1){
                            canvas_context.drawImage(buildings, this.grid[t].type*16, 32, 16,16, this.grid[t].x, this.grid[t].y, 16,16)
                        }else{
                            canvas_context.drawImage(buildings, this.grid[t].type*16, 0, 16,16, this.grid[t].x, this.grid[t].y, 16,16)
                        }
                    }else{
                        if(t%this.width == 0){
                            canvas_context.drawImage(buildings, this.grid[t].type*16, 64, 16,16, this.grid[t].x, this.grid[t].y, 16,16)
                        }else if(t%this.width == this.width-1){
                            canvas_context.drawImage(buildings, this.grid[t].type*16, 80, 16,16, this.grid[t].x, this.grid[t].y, 16,16)
                        }else{
                            canvas_context.drawImage(buildings, this.grid[t].type*16, 48, 16,16, this.grid[t].x, this.grid[t].y, 16,16)
                        }
                    }
                    if(wrek.ball.isPointInside(this.grid[t])){
                        let link = new LineOP(wrek.ball, this.grid[t])
                        this.grid[t].xmom = -Math.cos(link.angle())*5
                        this.grid[t].ymom = -Math.sin(link.angle())*5
                    }
                }
            //         this.xpointer+=16
            //         }
    ...
    class Wrecker{
        constructor(){
            this.string = []
            this.ball = new Circle(640, 150, 30, "gray")
            this.nodes = [this.ball]
            for(let t = 0;t<10;t++){
                let node = new Circle(this.ball.x-(t*12), this.ball.y, 5, "gray")
                this.nodes.push(node)
            }
            for(let t = 0;t<this.nodes.length;t++){
                for(let k = 0;k<this.nodes.length;k++){
                    if(t!=k){
                        let link = new LineOP(this.nodes[t], this.nodes[k])
                        if(link.hypotenuse()<15){
                            let spring = new SpringOP(this.nodes[t], this.nodes[k], 12, 2, "white")
                            this.string.push(spring)
                        }
                    }
                }
            }
        ...
        draw(){
            this.nodes[this.nodes.length-1].xmom += (TIP_engine.x-this.nodes[this.nodes.length-1].x)/4
            this.nodes[this.nodes.length-1].ymom += (TIP_engine.y-this.nodes[this.nodes.length-1].y)/4
            this.ball.ymom += 3
            for(let t = 0;t<this.string.length;t++){
                this.string[t].balance()
            }
            for(let t = 0;t<this.string.length;t++){
                this.string[t].balance()
            }
    ...
    let wrek = new Wrecker()

    function main() {
        canvas_context.clearRect(0, 0, canvas.width, canvas.height)  // refreshes the image
        gamepadAPI.update() //checks for button presses/stick movement on the connected controller)
        // // game code goes here
        for(let t=0;t<town.length;t++){
            town[t].draw()
        }
        wrek.draw()
        if(keysPressed['x']){
